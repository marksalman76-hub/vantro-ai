...
proxy_base = r'''import { NextRequest, NextResponse } from "next/server";
...
write("frontend/src/app/api/billing-refund-request/route.ts", proxy_base + r'''
export async function POST(req: NextRequest) {
  const body = await req.text();
  const upstream = await fetch(`${BACKEND_URL}/billing/refund-request`, {
    method: "POST",
    cache: "no-store",
...
write("frontend/src/app/api/billing-refund-request-status/route.ts", proxy_base + r'''
...
write("frontend/src/app/api/admin-billing-refund-requests/route.ts", proxy_base + r'''
export async function GET(req: NextRequest) {
...
write("frontend/src/app/api/admin-billing-refund-decision/route.ts", proxy_base + r'''
export async function POST(req: NextRequest) {
  const body = await req.text();
  const upstream = await fetch(`${BACKEND_URL}/admin/billing/refund-decision`, {
    method: "POST",
    cache: "no-store",
    headers: adminHeaders(req),
    body,
...
write("frontend/src/app/api/admin-billing-refund-execute/route.ts", proxy_base + r'''
export async function POST(req: NextRequest) {
  const body = await req.text();
  const upstream = await fetch(`${BACKEND_URL}/admin/billing/refund-execute`, {
    method: "POST",
    cache: "no-store",
    headers: adminHeaders(req),
    body,
...
write("test_governed_refund_workflow.py", r'''import os
import tempfile
...
print("REFUND_FRONTEND_PROXIES_AND_TESTS_INSTALLED")