...
BACKUP_DIR = ROOT / "backups" / f"governed_activation_persistence_routes_before_{STAMP}"
BACKUP_DIR.mkdir(parents=True, exist_ok=True)

main_path = ROOT / "backend" / "app" / "main.py"
test_path = ROOT / "test_governed_activation_persistence_routes.py"
...
if "from backend.app.runtime.governed_activation_persistence import" not in text:
    marker = "from fastapi"
    ...
    if idx == -1:
        ...
        line_end = text.find("\n", idx)
        text = text[:line_end + 1] + import_block + text[line_end + 1:]

if "/governed-activation-persistence/status" not in text:
    text = text.rstrip() + "\n\n" + route_block + "\n"
...
print("GOVERNED_ACTIVATION_PERSISTENCE_ROUTES_WIRED")
