...
WORKER_FILE = CORE / "execution_queue_worker_runtime.py"

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
main_text = MAIN.read_text(encoding="utf-8")

backup = BACKUPS / f"main_before_priority4_queue_worker_{timestamp}.py"
...
WORKER_FILE.write_text(worker_code.strip() + "\n", encoding="utf-8")

import_line = "from backend.app.core.execution_queue_worker_runtime import queue_worker_health, worker_heartbeat, claim_execution_queue_batch, run_queue_worker_once\n"
...
if "/admin/execution-queue/worker-health" not in main_text:
    main_text = main_text.rstrip() + "\n\n" + routes + "\n"

MAIN.write_text(main_text, encoding="utf-8")

print("PRIORITY4_QUEUE_WORKER_RUNTIME_INSTALLED")
print(f"Backup: {backup}")
print(f"Created/updated: {WORKER_FILE}")
print("Routes:")
print("/admin/execution-queue/worker-health")
print("/admin/execution-queue/worker-heartbeat")
print("/admin/execution-queue/claim-batch")
print("/admin/execution-queue/run-worker-once")