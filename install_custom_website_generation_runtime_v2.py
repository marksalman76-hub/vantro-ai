...
ROOT = Path.cwd()
RUNTIME = ROOT / "backend" / "app" / "runtime" / "custom_website_generation_runtime.py"
ADAPTER = ROOT / "backend" / "app" / "runtime" / "action_adapter_execution_layer.py"
PUBLIC_GENERATED = ROOT / "frontend" / "public" / "generated-sites"

BACKUP = ROOT / "backups" / f"custom_website_generation_runtime_v2_before_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
BACKUP.mkdir(parents=True, exist_ok=True)
shutil.copy2(ADAPTER, BACKUP / "action_adapter_execution_layer.py")
...
if "from backend.app.runtime.custom_website_generation_runtime import generate_custom_website_project" not in s:
    s = s.replace(
...
new = '''        generated_site = generate_custom_website_project(
...
if old not in s:
    raise SystemExit("Could not find existing website_draft_page_adapter action/output block.")

s = s.replace(old, new)

ADAPTER.write_text(s, encoding="utf-8")

print("CUSTOM_WEBSITE_GENERATION_RUNTIME_V2_INSTALLED")
print("Runtime:", RUNTIME)
print("Public generated:", PUBLIC_GENERATED)
print("Backup:", BACKUP)