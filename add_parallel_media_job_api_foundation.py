...
ROOT = Path.cwd()
route_dir = ROOT / "frontend" / "src" / "app" / "api" / "parallel-media-jobs"
route_file = route_dir / "route.ts"

backup_dir = ROOT / "backups" / f"parallel_media_job_api_foundation_before_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
backup_dir.mkdir(parents=True, exist_ok=True)

if route_file.exists():
    backup_target = backup_dir / "route.ts"
    backup_target.write_text(route_file.read_text(encoding="utf-8"), encoding="utf-8")
...
print("PARALLEL_MEDIA_JOB_API_FOUNDATION_ADDED")
print(f"Created/updated: {route_file}")
