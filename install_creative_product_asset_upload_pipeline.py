...
RUNTIME_FILE = ROOT / "backend" / "app" / "runtime" / "creative_product_asset_library.py"
MAIN_FILE = ROOT / "backend" / "app" / "main.py"

backup_dir = ROOT / "backups" / f"creative_product_asset_upload_pipeline_before_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
backup_dir.mkdir(parents=True, exist_ok=True)

for file_path in [RUNTIME_FILE, MAIN_FILE]:
    if file_path.exists():
        (backup_dir / file_path.name).write_text(file_path.read_text(encoding="utf-8"), encoding="utf-8")

RUNTIME_FILE.write_text(r'''from __future__ import annotations
...
import_block = "from backend.app.runtime.creative_product_asset_library import upload_creative_product_asset, list_creative_product_assets, delete_creative_product_asset, build_creative_execution_asset_context, creative_product_asset_library_status\n"
if import_block not in main:
    marker = "from fastapi import"
    if marker in main:
        # Keep imports local in routes too, but status import is safe.
        pass
...
if "/admin/creative/product-assets/upload" not in main:
    MAIN_FILE.write_text(main.rstrip() + "\n" + routes + "\n", encoding="utf-8")
else:
    MAIN_FILE.write_text(main, encoding="utf-8")

print("CREATIVE_PRODUCT_ASSET_UPLOAD_PIPELINE_INSTALLED")
print("Updated:", RUNTIME_FILE)
