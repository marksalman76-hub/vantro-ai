...
TEST = ROOT / "test_direct_media_provider_execution_runtime.py"

STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
BACKUP_DIR = ROOT / "backups" / f"direct_media_provider_execution_lane_before_{STAMP}"
...
if "from fastapi import Request" not in main:
    if "from fastapi import" in main:
        main = main.replace("from fastapi import", "from fastapi import Request,", 1)
    else:
        main = "from fastapi import Request\n" + main
...
if "DIRECT_MEDIA_PROVIDER_EXECUTION_LANE_V1" not in main:
    main = main.rstrip() + "\n" + route_block + "\n"
...
print("DIRECT_MEDIA_PROVIDER_EXECUTION_LANE_INSTALLED")
