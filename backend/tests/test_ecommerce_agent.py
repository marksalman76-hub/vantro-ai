...
@pytest.mark.unit
class TestEcommerceAgentPrompt:

    def test_prompt_exists(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        assert "ecommerce_agent" in AGENT_SYSTEM_PROMPTS

    def test_prompt_is_nonempty(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"]
    ...
    def test_prompt_has_quick_win_checklist_section(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"]
        assert "QUICK WIN CHECKLIST" in prompt.upper()

    def test_quick_win_mentions_48_hours(self):
        """QUICK WIN CHECKLIST must constrain items to under 48 hours."""
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"]
        assert "48" in prompt, "QUICK WIN CHECKLIST should reference the 48-hour execution window"

    def test_prompt_has_no_price_change_rule(self):
        """Prompt must contain an explicit no-live-price-change rule."""
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"].lower()
        assert "price change" in prompt or "price changes" in prompt
        assert "owner approval" in prompt

    def test_prompt_has_discount_approval_rule(self):
        """Prompt must require owner approval before promotional discounts are activated."""
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"].lower()
        assert "promotional discount" in prompt or "discount" in prompt
        assert "owner approval" in prompt

    def test_prompt_has_all_three_confidence_labels(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"]
    ...
    def test_prompt_has_hitl_gate_language(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"].upper()
        assert "HITL" in prompt or "REQUIRES REVIEW" in prompt or "DRAFT" in prompt

    def test_prompt_has_hitl2_designation(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"]
    ...
    def test_get_agent_system_prompt_helper(self):
        ...
        prompt = get_agent_system_prompt("ecommerce_agent")
        assert "Ecommerce Agent" in prompt
    ...
    def test_product_claims_compliance_rule(self):
        """Prompt must reference product claims accuracy / advertising standards."""
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"].lower()
        assert "product claims" in prompt or "advertising standards" in prompt or "accurate" in prompt

    def test_mobile_optimisation_referenced(self):
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"].lower()
...
@pytest.mark.unit
class TestFinancialActionScannerEcommerce:
...
## QUICK WIN CHECKLIST [DRAFT]
Action: Add trust badge below Add to Cart button.
ROI estimate: 0.3-0.8% conversion uplift [INDUSTRY AVERAGE]
Effort: [LOW]
"""
        violations = scan_for_financial_actions(clean)
        assert violations == []

    def test_price_change_approved_phrase_detected(self):
        """Scanner catches 'i have approved' constructs, which covers agent-driven approvals."""
        from app.agents.agent_executor import scan_for_financial_actions
        dirty = "I have approved the price reduction of $5 on all SKUs in the store."
        violations = scan_for_financial_actions(dirty)
        assert len(violations) > 0

    def test_authorisation_phrase_detected(self):
        from app.agents.agent_executor import scan_for_financial_actions
        dirty = "I have authorized the promotional discount of 20% on all items."
        violations = scan_for_financial_actions(dirty)
        assert len(violations) > 0

    def test_purchase_phrase_detected(self):
        from app.agents.agent_executor import scan_for_financial_actions
        dirty = "I have purchased the Shopify app on your behalf."
        violations = scan_for_financial_actions(dirty)
        assert len(violations) > 0

    def test_budget_allocated_phrase_detected(self):
        from app.agents.agent_executor import scan_for_financial_actions
        dirty = "Budget allocated: $2,000 for the Black Friday ad campaign."
        violations = scan_for_financial_actions(dirty)
        assert len(violations) > 0

    def test_spend_approved_phrase_detected(self):
        from app.agents.agent_executor import scan_for_financial_actions
        dirty = "Spend approved. Discount codes activated."
        violations = scan_for_financial_actions(dirty)
        assert len(violations) > 0

    def test_case_insensitive_matching(self):
        from app.agents.agent_executor import scan_for_financial_actions
        dirty = "I HAVE AUTHORIZED the payment of the influencer fee."
        violations = scan_for_financial_actions(dirty)
        assert len(violations) > 0
...
@pytest.mark.unit
class TestEcommerceExecutorGuardInjection:
    ...
    def test_financial_constraint_block_injected(self):
        ...
        def mock_anthropic_call(system_prompt, user_message, **kwargs):
            ...
            return "Clean ecommerce audit output with no financial actions.", 100, 50
            ...
            with patch("app.agents.agent_executor._call_anthropic", side_effect=mock_anthropic_call):
                with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-test-key"}):
                    execute_agent(
                        ...
                        system_prompt="You are the Ecommerce Agent.",
                        user_prompt="Audit my Shopify store.",
                    )
    ...
    def test_financial_constraint_comes_before_agent_prompt(self):
        ...
        def mock_call(system_prompt, user_message, **kwargs):
            ...
            return "Store audit output.", 100, 50

        with patch("app.agents.agent_executor._call_anthropic", side_effect=mock_call):
            with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-test-key"}):
                execute_agent(
                    agent_id="ecommerce_agent",
                    system_prompt="ECOMMERCE_AGENT_SPECIFIC_CONTENT_MARKER",
                    user_prompt="Test",
                )

        sys_prompt = captured[0]
        fc_pos = sys_prompt.index(FINANCIAL_CONSTRAINT_BLOCK[:30])
        agent_pos = sys_prompt.index("ECOMMERCE_AGENT_SPECIFIC_CONTENT_MARKER")
    ...
    def test_context_injected_into_user_message(self):
        ...
        def mock_call(system_prompt, user_message, **kwargs):
            ...
            return "Ecommerce audit output.", 100, 50
        ...
        with patch("app.agents.agent_executor._call_anthropic", side_effect=mock_call):
            with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-test-key"}):
                execute_agent(
                    agent_id="ecommerce_agent",
                    system_prompt="You are the Ecommerce Agent.",
                    user_prompt="Audit this store.",
                    context={"workspace": "Beach Gear Co", "industry": "Outdoor DTC"},
                )

        msg = captured_messages[0]
        assert "Beach Gear Co" in msg
        assert "Outdoor DTC" in msg

    def test_financial_violation_in_output_flagged(self):
        from app.agents.agent_executor import execute_agent

        def mock_call(system_prompt, user_message, **kwargs):
            return "I have authorized the promotional discount of 30% on all products.", 100, 50
    ...
    def test_clean_output_returns_no_violations(self):
        ...
        def mock_call(system_prompt, user_message, **kwargs):
            return "Store audit with no financial authorisations. All price changes require owner approval.", 200, 300
    ...
    def test_openai_fallback_when_anthropic_fails(self):
        ...
        def openai_ok(system_prompt, user_message):
            return "OpenAI fallback ecommerce output.", 100, 50
        ...
        assert text == "OpenAI fallback ecommerce output."
...
@pytest.mark.unit
class TestEcommerceAgentGuardrails:
    ...
    def test_ecommerce_agent_is_hitl2_not_hitl3(self):
        """ecommerce_agent is HITL-2 — must never be HITL-3 gated."""
        from app.agents.agent_registry import AGENT_CATALOGUE
        assert AGENT_CATALOGUE["ecommerce_agent"]["hitl_default"] == "HITL-2"
        assert AGENT_CATALOGUE["ecommerce_agent"]["hitl_default"] != "HITL-3"

    def test_no_live_price_change_rule_in_prompt(self):
        """The absolute no-live-price-change rule must be present and unambiguous."""
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"].lower()
    ...
    def test_promotional_discount_requires_approval_in_prompt(self):
        """Every promotional discount recommendation must require owner approval."""
        from app.agents.agent_prompts import AGENT_SYSTEM_PROMPTS
        prompt = AGENT_SYSTEM_PROMPTS["ecommerce_agent"]
        assert "REQUIRES OWNER APPROVAL — PROMOTIONAL DISCOUNT" in prompt
    ...
    def test_secrets_in_prompt_not_in_output(self):
        ...
        def mock_call(system_prompt, user_message, **kwargs):
            return "I cannot process credentials. Please provide a different task.", 50, 30
...
@pytest.mark.unit
class TestEcommerceAgentAPI:

    def test_run_ecommerce_agent_unauthenticated(self, client):
        resp = client.post(
            "/api/agents/ecommerce_agent/run",
            json={"prompt": "Audit my Shopify store for conversion improvements."},
        )
        assert resp.status_code == status.HTTP_401_UNAUTHORIZED

    def test_run_ecommerce_agent_missing_prompt(self, client, authenticated_client):
        resp = authenticated_client.post("/api/agents/ecommerce_agent/run", json={})
    ...
    def test_run_ecommerce_agent_empty_prompt(self, client, authenticated_client):
    ...
    def test_run_ecommerce_agent_submits_job(self, client, authenticated_client):
        ...
        with patch("app.agents.agent_executor.execute_agent") as mock_exec:
            ...
            resp = authenticated_client.post(
                "/api/agents/ecommerce_agent/run",
                json={"prompt": "Audit my store and give me a quick win checklist."},
            )
...
@pytest.mark.unit
class TestEcommerceOutputFormatValidator:
    """Tests for the has_all_sections helper and expected ecommerce output structure."""

    def test_good_output_passes(self):
        good = "\n".join(f"## {s}" for s in REQUIRED_SECTIONS)
        assert has_all_sections(good) == []

    def test_missing_quick_win_checklist_detected(self):
        partial = (
            "## STORE AUDIT\n"
            "## PRODUCT CATALOGUE STRATEGY\n"
            "## PRICING STRATEGY\n"
        ...
        assert "QUICK WIN CHECKLIST" in missing

    def test_missing_abandoned_cart_detected(self):
        partial = (
            "## STORE AUDIT\n"
            "## PRODUCT CATALOGUE STRATEGY\n"
            "## PRICING STRATEGY\n"
        ...
        missing = has_all_sections(partial)
        assert "ABANDONED CART RECOVERY" in missing
    ...
    def test_confidence_labels_in_mock_output(self):
        output = """
## STORE AUDIT [DRAFT]
Conversion rate: 2.1% [VERIFIED DATA] pulled from Google Analytics.
Industry average cart abandonment: 70% [INDUSTRY AVERAGE]
Expected uplift from trust badge addition: 0.5% [PROJECTED - VALIDATE]
"""
        found = [label for label in REQUIRED_CONFIDENCE_LABELS if label in output]
        assert len(found) == 3

    def test_hitl_draft_label_in_audit_output(self):
        output = "## STORE AUDIT [DRAFT]\nConversion rate 2.3%"
        assert "[DRAFT]" in output

    def test_hitl_review_label_in_pricing_output(self):
        output = "## PRICING STRATEGY [REQUIRES REVIEW]\nEntry price Â£29 [PROJECTED - VALIDATE]"
        assert "[REQUIRES REVIEW]" in output

    def test_owner_approval_label_in_discount_output(self):
        output = "Promotional discount of 15% off: [REQUIRES OWNER APPROVAL — PROMOTIONAL DISCOUNT]"
        assert "REQUIRES OWNER APPROVAL" in output

    def test_quick_win_mentions_48h(self):
        output = "## QUICK WIN CHECKLIST\nAll items executable in under 48 hours with zero dev work."
        assert "48" in output
