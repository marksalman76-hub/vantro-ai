...
_ADMIN_EMAIL = _os.getenv("ADMIN_EMAIL", "")
...
async def get_admin_stats(
    ...
    processing_jobs = db.query(func.count(MediaJob.id)).filter(MediaJob.status == "processing").scalar()
    pending_jobs = db.query(func.count(MediaJob.id)).filter(MediaJob.status == "pending").scalar()
    failed_jobs = db.query(func.count(MediaJob.id)).filter(MediaJob.status == "failed").scalar()
    return {
        "total_users": total_users,
        "active_subscriptions": active_subs,
        "total_media_jobs": total_jobs,
        "completed_media_jobs": completed_jobs,
        "processing_media_jobs": processing_jobs,
        "pending_media_jobs": pending_jobs,
        "failed_media_jobs": failed_jobs,
    }
