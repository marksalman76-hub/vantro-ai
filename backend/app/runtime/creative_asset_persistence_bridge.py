...
def _lightweight_index_record(record):
    return {
        ...
        "job_id": _truncate(record.get("job_id"), 240),
        "task_id": _truncate(record.get("task_id"), 240),
        "durable_queue_job_id": _truncate(record.get("durable_queue_job_id"), 240),
        "media_pack_id": _truncate(record.get("media_pack_id"), 240),
        "correlation": record.get("correlation") if isinstance(record.get("correlation"), dict) else {},
...
def persist_creative_asset(asset_packet: dict):
    ...
    stored = {
        ...
        "job_id": _truncate(packet.get("job_id") or packet.get("media_job_id"), 240),
        "task_id": _truncate(packet.get("task_id") or packet.get("media_job_id") or packet.get("job_id"), 240),
        "durable_queue_job_id": _truncate(packet.get("durable_queue_job_id"), 240),
        "media_pack_id": _truncate(packet.get("media_pack_id"), 240),
    ...
    if record_canonical_media_asset is not None:
        ...
        "durable_queue_job_id": stored.get("durable_queue_job_id"),
        "media_pack_id": stored.get("media_pack_id"),
        "correlation": stored.get("correlation") if isinstance(stored.get("correlation"), dict) else {},
        ...
        if canonical.get("success") and canonical_assets:
            ...
            for item in canonical_assets[: int(limit or 100)]:
                clean = {
                    ...
                    "tenant_id": item.get("tenant_id") or (item.get("payload") or {}).get("tenant_id"),
                    "media_job_id": item.get("media_job_id") or (item.get("payload") or {}).get("media_job_id"),
                    "job_id": item.get("job_id") or (item.get("payload") or {}).get("job_id"),
                    "task_id": item.get("task_id") or (item.get("payload") or {}).get("task_id"),
                    "durable_queue_job_id": item.get("durable_queue_job_id") or (item.get("payload") or {}).get("durable_queue_job_id"),
                    "media_pack_id": item.get("media_pack_id") or (item.get("payload") or {}).get("media_pack_id"),
                    "correlation": (item.get("payload") or {}).get("correlation") if isinstance((item.get("payload") or {}).get("correlation"), dict) else {},
