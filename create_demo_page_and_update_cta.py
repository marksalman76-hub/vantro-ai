...
DEMO_PAGE = ROOT / "frontend" / "src" / "app" / "demo" / "page.tsx"
BACKUP_DIR = ROOT / "backups"
BACKUP_DIR.mkdir(exist_ok=True)

stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
backup = BACKUP_DIR / f"page_before_demo_cta_{stamp}.tsx"
shutil.copy2(PAGE, backup)

s = PAGE.read_text(encoding="utf-8")

# Replace visible CTA wording.
s = s.replace("Start free", "Demo")
s = s.replace("Start creating free", "Demo")
s = s.replace("Start creating", "Demo")
s = s.replace("Start free — no card needed", "Demo")

# Route demo CTAs to /demo.
s = re.sub(r'href="/signup"([^>]*>\s*Demo)', r'href="/demo"\1', s)
s = re.sub(r'href="/activate"([^>]*>\s*Demo)', r'href="/demo"\1', s)

PAGE.write_text(s, encoding="utf-8")

DEMO_PAGE.parent.mkdir(parents=True, exist_ok=True)
DEMO_PAGE.write_text(r'''"use client";
...
print("DEMO_PAGE_CREATED_AND_CTA_UPDATED")
