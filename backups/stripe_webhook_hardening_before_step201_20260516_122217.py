...
def verify_stripe_signature(raw_body: bytes, signature_header: Optional[str], webhook_secret: Optional[str]) -> Dict[str, Any]:
    """
    Hardened signature verification wrapper.

    In production, STRIPE_WEBHOOK_SECRET must be configured.
    This function supports local/dev testing by returning configured=False
    instead of falsely accepting a signature.
    """
    ...
    try:
        ...
        return {
            ...
            "configured": True,
            "reason": "signature_verified" if hmac.compare_digest(expected, received_signature) else "signature_mismatch",
        }
