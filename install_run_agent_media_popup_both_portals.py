...
TEST = ROOT / "test_run_agent_media_popup_both_portals.py"
OLD_CLIENT_PANEL_TEST = ROOT / "test_client_universal_complete_media_run_agent_panel.py"

STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
BACKUP_DIR = ROOT / "backups" / f"run_agent_media_popup_both_portals_before_{STAMP}"
BACKUP_DIR.mkdir(parents=True, exist_ok=True)

for path in [COMPONENT, ADMIN_PAGE, TEST, OLD_CLIENT_PANEL_TEST]:
    if path.exists():
...
if "ADMIN_RUN_AGENT_MEDIA_POPUP_V1" not in admin:
    admin = admin.replace(
...
if "ADMIN_RUN_AGENT_COMPLETE_MEDIA_ROUTE_V1" not in admin:
    insert_before = "  const navItems = ["
    admin_media_function = r'''
...
'''
    if insert_before not in admin:
        raise SystemExit("ADMIN_NAV_ITEMS_ANCHOR_NOT_FOUND")
    admin = admin.replace(insert_before, admin_media_function + insert_before, 1)
...
OLD_CLIENT_PANEL_TEST.write_text(
r'''from pathlib import Path
...
print("RUN_AGENT_MEDIA_POPUP_BOTH_PORTALS_INSTALLED")
print(f"Backup: {BACKUP_DIR}")
print(f"Updated: {COMPONENT}")
print(f"Updated: {ADMIN_PAGE}")
print(f"Updated: {TEST}")
print(f"Updated: {OLD_CLIENT_PANEL_TEST}")