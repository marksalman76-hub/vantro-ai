...
SECURITY_FILE = CORE_DIR / "security_hardening_runtime.py"
...
if not MAIN.exists():
    raise FileNotFoundError(f"Missing main.py at {MAIN}")

main_text = MAIN.read_text(encoding="utf-8")
backup_path = BACKUP_DIR / f"main_before_priority5_security_hardening_{timestamp}.py"
backup_path.write_text(main_text, encoding="utf-8")
...
SECURITY_FILE.write_text(security_runtime.strip() + "\n", encoding="utf-8")
...
if "install_security_hardening(app)" not in main_text:
    app_marker = "app = FastAPI("
    idx = main_text.find(app_marker)
    if idx == -1:
        app_marker = "app=FastAPI("
        idx = main_text.find(app_marker)

    if idx == -1:
        raise RuntimeError("Could not find FastAPI app creation in main.py")

    # Insert after the first line following app definition block.
    # Safer path: place after the first standalone closing parenthesis after app = FastAPI(...)
    start = idx
    close_idx = main_text.find("\n)", start)
    if close_idx != -1:
        ...
        if line_end == -1:
            line_end = len(main_text)
        main_text = main_text[:line_end] + "\ninstall_security_hardening(app)" + main_text[line_end:]
...
if "/admin/security-hardening-readiness" not in main_text:
    main_text = main_text.rstrip() + route_block + "\n"

MAIN.write_text(main_text, encoding="utf-8")

print("PRIORITY5_SECURITY_HARDENING_RUNTIME_INSTALLED")
print(f"Backup: {backup_path}")
print(f"Created/updated: {SECURITY_FILE}")
print("Route: /admin/security-hardening-readiness")