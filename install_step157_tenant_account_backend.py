...
AUTH_FILE = CORE_DIR / "tenant_account_runtime.py"

CORE_DIR.mkdir(parents=True, exist_ok=True)
DATA_DIR.mkdir(parents=True, exist_ok=True)

if not MAIN.exists():
    raise FileNotFoundError(f"Missing backend main file: {MAIN}")

AUTH_CODE = r'''from __future__ import annotations
...
if "tenant_account_runtime" not in main_text:
    main_text = import_block + "\n" + main_text

if "/admin/client-activation-invite" not in main_text:
    main_text = main_text.rstrip() + "\n" + routes_block + "\n"

MAIN.write_text(main_text, encoding="utf-8")

print("STEP_157_TENANT_ACCOUNT_BACKEND_INSTALLED")
print("auth_file", AUTH_FILE)
print("main_file", MAIN)
print("routes_added", True)
print("password_storage", "pbkdf2_sha256_hash_only")
print("activation_links", "one_time_use")
print("STEP_157_OK")