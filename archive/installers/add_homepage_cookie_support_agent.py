...
client = root / "frontend" / "src" / "app" / "homepage-support-client.tsx"
backup_dir = root / "backups"
backup_dir.mkdir(exist_ok=True)

stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
backup = backup_dir / f"homepage_before_cookie_support_agent_{stamp}.tsx"

s = page.read_text(encoding="utf-8")
backup.write_text(s, encoding="utf-8")

if 'import HomepageSupportClient from "./homepage-support-client";' not in s:
    s = s.replace('"use client";\n', '"use client";\n\nimport HomepageSupportClient from "./homepage-support-client";\n', 1)

if "<HomepageSupportClient />" not in s:
    s = s.replace("      <Footer />", "      <Footer />\n      <HomepageSupportClient />", 1)
...
print("HOMEPAGE_COOKIE_AND_SUPPORT_AGENT_ADDED")
print("Backup:", backup)