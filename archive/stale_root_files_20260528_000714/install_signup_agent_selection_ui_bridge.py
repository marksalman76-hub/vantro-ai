...
signup_page = ROOT / "frontend" / "src" / "app" / "signup" / "page.tsx"
component_path = ROOT / "frontend" / "src" / "components" / "SignupAgentSelectionBridge.tsx"
test_file = ROOT / "test_signup_agent_selection_ui_bridge.py"

backup_dir = ROOT / "backups" / f"signup_agent_selection_ui_bridge_before_{STAMP}"
backup_dir.mkdir(parents=True, exist_ok=True)

for p in [signup_page, component_path, test_file]:
    if p.exists():
        backup_name = str(p.relative_to(ROOT)).replace("\\", "__").replace("/", "__")
        (backup_dir / backup_name).write_text(p.read_text(encoding="utf-8"), encoding="utf-8")

if not signup_page.exists():
    raise FileNotFoundError(f"Missing signup page: {signup_page}")

component_path.parent.mkdir(parents=True, exist_ok=True)

component_path.write_text(r'''
"use client";
...
page_text = signup_page.read_text(encoding="utf-8")

if "SignupAgentSelectionBridge" not in page_text:
    if page_text.lstrip().startswith('"use client";') or page_text.lstrip().startswith("'use client';"):
        lines = page_text.splitlines()
        insert_index = 1
        while insert_index < len(lines) and lines[insert_index].strip() == "":
            insert_index += 1
        lines.insert(insert_index, 'import SignupAgentSelectionBridge from "@/components/SignupAgentSelectionBridge";')
        page_text = "\n".join(lines)
    else:
        page_text = 'import SignupAgentSelectionBridge from "@/components/SignupAgentSelectionBridge";\n' + page_text
    ...
    for target_marker in preferred_targets:
        idx = page_text.find(target_marker)
        if idx != -1:
            page_text = page_text[:idx] + "        <SignupAgentSelectionBridge />\n" + page_text[idx:]
            inserted = True
            break

    if not inserted:
        raise SystemExit("SIGNUP_PAGE_INSERT_TARGET_NOT_FOUND")

    signup_page.write_text(page_text, encoding="utf-8")
    print("SIGNUP_AGENT_SELECTION_COMPONENT_INSERTED")
else:
    print("SIGNUP_AGENT_SELECTION_COMPONENT_ALREADY_PRESENT")
...
print("SIGNUP_AGENT_SELECTION_UI_BRIDGE_INSTALLED")
print(f"Backup folder: {backup_dir}")
print(f"Created/updated: {component_path}")
print(f"Updated: {signup_page}")
