...
SESSION_FILE = CORE / "session_auth_hardening_runtime.py"
...
backup = BACKUPS / f"main_before_priority5_session_auth_hardening_{timestamp}.py"
...
SESSION_FILE.write_text(session_runtime.strip() + "\n", encoding="utf-8")

import_line = "from backend.app.core.session_auth_hardening_runtime import install_session_auth_hardening, session_auth_hardening_readiness\n"
...
if "install_session_auth_hardening(app)" not in main_text:
    marker = "install_security_hardening(app)"
    idx = main_text.find(marker)
    if idx != -1:
        insert_at = idx + len(marker)
        main_text = main_text[:insert_at] + "\ninstall_session_auth_hardening(app)" + main_text[insert_at:]
    else:
        ...
        if idx == -1:
            ...
            idx = main_text.find(marker)
        if idx == -1:
            raise RuntimeError("Could not find safe insertion point for session auth middleware")
        main_text = main_text[:idx] + "install_session_auth_hardening(app)\n" + main_text[idx:]
...
if "/admin/session-auth-hardening-readiness" not in main_text:
    main_text = main_text.rstrip() + "\n\n" + route + "\n"

MAIN.write_text(main_text, encoding="utf-8")

print("PRIORITY5_SESSION_AUTH_HARDENING_INSTALLED")
print(f"Backup: {backup}")
print(f"Created/updated: {SESSION_FILE}")
print("Route: /admin/session-auth-hardening-readiness")