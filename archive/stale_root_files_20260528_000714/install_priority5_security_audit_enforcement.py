...
AUDIT_FILE = CORE / "security_audit_enforcement_runtime.py"

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

if not MAIN.exists():
    raise FileNotFoundError(f"Missing main.py: {MAIN}")

main_text = MAIN.read_text(encoding="utf-8")
backup = BACKUPS / f"main_before_priority5_security_audit_enforcement_{timestamp}.py"
...
AUDIT_FILE.write_text(audit_runtime.strip() + "\n", encoding="utf-8")

import_line = "from backend.app.core.security_audit_enforcement_runtime import install_security_audit_enforcement, security_audit_enforcement_readiness\n"
...
if import_line not in main_text:
    ...
    if idx == -1:
        ...
        idx = main_text.find(marker)
    if idx == -1:
        raise RuntimeError("Could not find FastAPI app marker in main.py")
    main_text = main_text[:idx] + "\n" + import_line + main_text[idx:]

if "install_security_audit_enforcement(app)" not in main_text:
    marker = "install_session_auth_hardening(app)"
    idx = main_text.find(marker)
    if idx != -1:
        insert_at = idx + len(marker)
        main_text = main_text[:insert_at] + "\ninstall_security_audit_enforcement(app)" + main_text[insert_at:]
    else:
        marker = "install_security_hardening(app)"
        idx = main_text.find(marker)
        if idx == -1:
            marker = "app.add_middleware("
            idx = main_text.find(marker)
        if idx == -1:
            raise RuntimeError("Could not find safe middleware insertion point")
        insert_at = idx + len(marker) if "install_security_hardening" in marker else idx
        main_text = main_text[:insert_at] + "\ninstall_security_audit_enforcement(app)" + main_text[insert_at:]
...
if "/admin/security-audit-enforcement-readiness" not in main_text:
    main_text = main_text.rstrip() + "\n\n" + route + "\n"

MAIN.write_text(main_text, encoding="utf-8")

print("PRIORITY5_SECURITY_AUDIT_ENFORCEMENT_INSTALLED")
print(f"Backup: {backup}")
print(f"Created/updated: {AUDIT_FILE}")
print("Route: /admin/security-audit-enforcement-readiness")