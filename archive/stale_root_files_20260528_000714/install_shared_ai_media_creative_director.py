...
runtime_file = runtime_dir / "ai_media_creative_director.py"
main_file = ROOT / "backend" / "app" / "main.py"

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

if runtime_file.exists():
    backup = BACKUPS / f"ai_media_creative_director_before_shared_install_{timestamp}.py"
    backup.write_text(runtime_file.read_text(encoding="utf-8"), encoding="utf-8")

if main_file.exists():
    main_backup = BACKUPS / f"main_before_shared_ai_media_creative_director_{timestamp}.py"
    main_backup.write_text(main_file.read_text(encoding="utf-8"), encoding="utf-8")
...
test_file = ROOT / "test_shared_ai_media_creative_director.py"
...
# Wire admin routes into main.py safely if main.py exists.
if main_file.exists():
    main_text = main_file.read_text(encoding="utf-8")

    import_line = "from backend.app.runtime.ai_media_creative_director import readiness as ai_media_creative_director_readiness, run_shared_ai_media_creative_director\n"
...
'''
    ...
    if "/admin/ai-media-creative-director/readiness" not in main_text:
        main_text = main_text.rstrip() + "\n" + route_block.strip() + "\n"

    main_file.write_text(main_text, encoding="utf-8")

print("SHARED_AI_MEDIA_CREATIVE_DIRECTOR_INSTALLED")
print(f"Created/updated: {runtime_file}")
print(f"Created/updated: {test_file}")
print("Routes:")
print("/admin/ai-media-creative-director/readiness")
print("/admin/ai-media-creative-director/run")
print("Scope: platform-wide reusable capability for all relevant agents")