...
backup = ROOT / "backups" / f"row10_all_agent_output_contracts_before_{stamp}"
backup.mkdir(parents=True, exist_ok=True)

contract_lib = ROOT / "frontend" / "src" / "lib" / "allAgentOutputContracts.ts"
...
contract_route = ROOT / "frontend" / "src" / "app" / "api" / "all-agent-output-contracts" / "route.ts"

for p in [contract_lib, delegated_route, latest_route, contract_route]:
    if p.exists():
        shutil.copy2(p, backup / p.name)

contract_lib.parent.mkdir(parents=True, exist_ok=True)

contract_lib.write_text(r'''export type AgentOutputContract = {
  agent_key: string;
  display_name: string;
  category: string;
...
if 'allAgentOutputContracts' not in delegated_text:
...
if 'allAgentOutputContracts' not in latest_text:
...
contract_route.parent.mkdir(parents=True, exist_ok=True)
contract_route.write_text(r'''import { NextRequest, NextResponse } from "next/server";
import {
...
test = ROOT / "test_row10_all_agent_output_contracts.py"
...
print("ROW10_ALL_AGENT_OUTPUT_CONTRACTS_INSTALLED")
print(f"Backup: {backup}")
print(f"Created/updated: {contract_lib}")
print(f"Updated: {delegated_route}")
print(f"Updated: {latest_route}")
print(f"Created/updated: {contract_route}")
