...
ROOT = Path.cwd()
WORKER = ROOT / "backend" / "app" / "core" / "execution_queue_worker_runtime.py"
...
if not WORKER.exists():
    raise FileNotFoundError(f"Missing worker runtime: {WORKER}")

backup = BACKUPS / f"execution_queue_worker_runtime_before_claim_lifecycle_fix_{timestamp}.py"
...
WORKER.write_text(worker_code.strip() + "\n", encoding="utf-8")

main = ROOT / "backend" / "app" / "main.py"
main_text = main.read_text(encoding="utf-8")
main_backup = BACKUPS / f"main_before_queue_worker_clear_locks_route_{timestamp}.py"
main_backup.write_text(main_text, encoding="utf-8")

import_old = "from backend.app.core.execution_queue_worker_runtime import queue_worker_health, worker_heartbeat, claim_execution_queue_batch, run_queue_worker_once\n"
import_new = "from backend.app.core.execution_queue_worker_runtime import queue_worker_health, worker_heartbeat, clear_queue_worker_locks, claim_execution_queue_batch, run_queue_worker_once\n"

if import_old in main_text and import_new not in main_text:
    main_text = main_text.replace(import_old, import_new)
...
if "/admin/execution-queue/clear-worker-locks" not in main_text:
    insert_marker = '@app.post("/admin/execution-queue/claim-batch")'
    idx = main_text.find(insert_marker)
    if idx != -1:
        main_text = main_text[:idx] + route + "\n" + main_text[idx:]
    else:
        main_text = main_text.rstrip() + "\n\n" + route + "\n"

main.write_text(main_text, encoding="utf-8")

print("PRIORITY4_QUEUE_WORKER_CLAIM_LIFECYCLE_FIXED")
print(f"Worker backup: {backup}")
print(f"Main backup: {main_backup}")
print("Route added: /admin/execution-queue/clear-worker-locks")