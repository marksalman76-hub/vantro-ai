...
| AWS-06 | ECS/Fargate media worker execution boundary | Completed additive local/no-op worker processing boundary that consumes AWS-05 accepted envelopes or AWS-03 queue messages; synthetic durable worker lifecycle proof now also covers queued representation, claim once, duplicate-claim block, processing, retry, terminal failure, terminal completion, terminal readback, DLQ/recovery shape, redacted admin diagnostics, client-safe status, and rollback blocking; synthetic failed-job recovery proof now covers failure classification, retry exhaustion, redacted DLQ reference shape, admin recovery representation, client-safe failed status, terminal failed readback, recovered/requeued state, and synthetic recovered/completed state. Existing worker loops, routes, direct media execution, providers, billing, and asset persistence remain unchanged. | Future ECS/Fargate media worker can process accepted jobs through a lifecycle with provider execution, duration-aware segmentation, ffmpeg composition, asset persistence, status updates, audit evidence, billing finalization, retry, and DLQ hooks. This row provides local/no-op worker processing and future SaaS worker model markers without live side effects or final 27-agent catalogue changes. | `backend/app/runtime/ecs_media_worker_execution_boundary.py`, `backend/app/runtime/synthetic_durable_worker_lifecycle.py`, `backend/app/runtime/synthetic_failed_job_recovery.py`, `verify_ecs_media_worker_execution_boundary.py`, `verify_synthetic_durable_worker_lifecycle.py`, `verify_synthetic_failed_job_recovery.py`, existing `backend/app/workers/media_worker.py` | High | `python -X utf8 verify_ecs_media_worker_execution_boundary.py`; `python -X utf8 verify_synthetic_durable_worker_lifecycle.py`; `python -X utf8 verify_synthetic_failed_job_recovery.py`; worker integration smoke only when live worker/claim behavior is intentionally wired | Worker boundary preserves queue/envelope fields, agent selection references, authority, provider preferences, duration/aspect, audit correlation, and admin/client safe views while requiring no AWS credentials and starting no worker loop, ECS/Fargate task, SQS polling, provider/API call, ffmpeg composition, asset persistence, Stripe/billing/credit finalization, or agent catalogue migration. Synthetic durable worker lifecycle proof records `synthetic_worker_lifecycle_attempted=true`, `claim_once_passed=true`, `duplicate_claim_blocked=true`, `retry_state_represented=true`, `dlq_or_recovery_shape_present=true`, `provider_call_attempted=false`, `media_generation_attempted=false`, `stripe_call_attempted=false`, `billing_mutation_attempted=false`, `credit_mutation_attempted=false`, and `public_cutover_enabled=false`. Synthetic failed-job and DLQ recovery proof records `synthetic_failed_job_recovery_attempted=true`, `failure_classification_passed=true`, `retry_exhaustion_represented=true`, `dlq_shape_present=true`, `dlq_reference_redacted=true`, `admin_recovery_action_represented=true`, `client_safe_failed_status_redacted=true`, `admin_diagnostics_redacted=true`, `recovered_or_requeued_state_represented=true`, `terminal_failed_readback_passed=true`, `terminal_recovered_or_completed_represented=true`, `provider_call_attempted=false`, `media_generation_attempted=false`, `stripe_call_attempted=false`, `billing_mutation_attempted=false`, `credit_mutation_attempted=false`, and `public_cutover_enabled=false`. |
...
| AWS-15 | Controlled AWS validation and API/status cutover readiness | AWS-15A controlled AWS environment validation is complete, AWS-15C local env loading is redacted, AWS-15E adds API/status route cutover boundary decision plumbing, AWS-16 wires backend acceptance/status route integration behind explicit flags, AWS-17 prepares durable repository/queue adapters in dry-run mode behind those same route gates, AWS-18 adds a rollback and kill-switch control boundary, AWS-19 adds an observability and incident diagnostics boundary, AWS-20 adds a controlled live AWS integration rehearsal boundary, the live synthetic durable handoff boundary proves one owner-approved DB/SQS/status handoff without changing production route behavior, synthetic durable worker lifecycle proof covers local claim/retry/fail/complete/DLQ shape without live worker execution, and synthetic failed-job recovery proof covers local retry exhaustion, DLQ/recovery, admin recovery, client-safe failure, and synthetic recovered/completed state. Live AWS route behavior remains disabled unless `AWS_OPTION_A_ENABLED`, `AWS_OPTION_A_ROUTE_CUTOVER_ENABLED`, and the operation-specific `AWS_OPTION_A_ACCEPTANCE_CUTOVER_ENABLED` or `AWS_OPTION_A_STATUS_CUTOVER_ENABLED` flag are enabled with IAM/RDS/SQS/S3/Secrets validation evidence; route mode alone still does not write RDS or send SQS, rollback controls can force compatibility fallback before durable repository and queue packets are prepared, observability only prepares sanitized diagnostics, AWS-20 rehearsal requires explicit rehearsal enabled + owner approved + per-resource rehearsal flags, live durable handoff requires separate explicit handoff flags, and future live rehearsals remain owner-approved only. | Remaining route work was compressed into AWS-17 through AWS-20; AWS-17 durable repository/queue adapters are now dry-run prepared, AWS-18 rollback controls are implemented/tested, AWS-19 observability is implemented/tested, AWS-20 safe default verifier passes, RDS rollback-only proof is proven with `insert_read_passed=true`, `update_read_passed=true`, and `transaction_rolled_back=true`, SQS-focused live send is proven with sanitized non-customer/non-executable proof, S3-focused marker write/read/delete cleanup is proven with sanitized proof, route-gated synthetic durable job handoff is proven in dry-run, live synthetic durable write/send/status handoff is proven with sanitized non-customer/non-executable proof, synthetic durable worker lifecycle is proven locally, synthetic failed-job recovery is proven locally, and AWS-20/live handoff/worker-lifecycle/recovery infrastructure proof is recorded without enabling production route cutover, live worker loops, providers, Stripe, billing, credits, customer traffic, public launch, or later migration rows. | `live_validate_aws_option_a_environment.py`, `verify_live_aws_option_a_environment_validation.py`, `backend/app/runtime/aws_option_a_route_cutover_boundary.py`, `verify_aws_option_a_route_cutover_boundary.py`, `backend/app/runtime/aws_option_a_route_integration.py`, `backend/app/runtime/aws_option_a_rollback_controls.py`, `backend/app/runtime/aws_option_a_observability.py`, `backend/app/runtime/aws_option_a_live_rehearsal.py`, `backend/app/runtime/aws_option_a_live_durable_handoff.py`, `backend/app/runtime/synthetic_durable_worker_lifecycle.py`, `backend/app/runtime/synthetic_failed_job_recovery.py`, `verify_aws_option_a_route_integration.py`, `verify_aws_option_a_durable_enqueue_dry_run.py`, `verify_route_gated_durable_job_handoff.py`, `verify_aws_option_a_rollback_controls.py`, `verify_aws_option_a_observability.py`, `verify_aws_option_a_live_rehearsal.py`, `verify_live_synthetic_durable_handoff.py`, `verify_synthetic_durable_worker_lifecycle.py`, `verify_synthetic_failed_job_recovery.py`, `backend/app/main.py` | High | `python -X utf8 live_validate_aws_option_a_environment.py`; `python -X utf8 verify_live_aws_option_a_environment_validation.py`; `python -X utf8 verify_aws_option_a_route_cutover_boundary.py`; `python -X utf8 verify_aws_option_a_route_integration.py`; `python -X utf8 verify_aws_option_a_durable_enqueue_dry_run.py`; `python -X utf8 verify_route_gated_durable_job_handoff.py`; `python -X utf8 verify_aws_option_a_rollback_controls.py`; `python -X utf8 verify_aws_option_a_observability.py`; `python -X utf8 verify_aws_option_a_live_rehearsal.py`; `python -X utf8 verify_live_synthetic_durable_handoff.py`; `python -X utf8 verify_synthetic_durable_worker_lifecycle.py`; `python -X utf8 verify_synthetic_failed_job_recovery.py`; owner-approved live handoff only with `AWS_OPTION_A_LIVE_DURABLE_HANDOFF_ENABLED=true`, `AWS_OPTION_A_LIVE_DURABLE_HANDOFF_OWNER_APPROVED=true`, `AWS_OPTION_A_LIVE_DURABLE_VALIDATION_CONFIRMED=true`, and DB/SQS/status/cleanup flags | AWS-15/AWS-16/AWS-17/AWS-18/AWS-19/AWS-20 now prove controlled validation has default no-live/no-network behavior, uses service-specific live validation flags, local env loading is redacted, backend route integration defaults to `compatibility_runtime_path`, existing route output remains unchanged unless diagnostics or AWS cutover intent are explicit, AWS cutover cannot activate from `AWS_OPTION_A_ENABLED` alone, acceptance/status cutovers are independently gated, validation evidence is required, durable repository and queue packets are prepared only when the route is ready and rollback controls are clear, route-gated durable handoff produces `durable_repository_dry_run_prepared`, `queue_enqueue_dry_run_prepared`, and `durable_status_read_dry_run_prepared` proof, live synthetic durable handoff proves DB write/read/status cleanup and one non-customer non-executable SQS send behind separate owner-approved flags, synthetic durable worker lifecycle proves claim once, duplicate-claim block, processing, retry, terminal failed/completed states, terminal readback, and DLQ/recovery shape without live worker/provider/billing side effects, synthetic failed-job recovery proves failure classification, retry exhaustion, redacted DLQ shape, admin recovery representation, client-safe failed status, terminal failed readback, recovered/requeued state, and synthetic recovered/completed state without live side effects, kill switch and force-fallback controls block durable write/send/recovery planning, observability prepares admin-safe diagnostic snapshots/client-safe support summaries/internal incident events without external logging, AWS-20 safe default mode blocks rehearsal unless owner-approved resource flags are present, live rehearsal artifacts are synthetic/non-customer/non-executable, RDS rollback path is proven, SQS send is proven, S3 write/read/delete cleanup is proven, AWS-20 is closed, admin/client views are separated, final 27-agent rules are preserved, and no worker/provider/Stripe/billing/credit/media mutation occurs. |

## AWS-20 Route-Gated Durable Handoff Proof Note

`verify_route_gated_durable_job_handoff.py` records the route-gated synthetic durable job handoff proof without adding a later migration row. It proves explicit AWS route flags are required, `durable_repository_dry_run_prepared`, `queue_enqueue_dry_run_prepared`, `durable_status_read_dry_run_prepared`, rollback-safe proof records, redacted admin diagnostics, client-safe status, rollback blocking, and no worker/provider/Stripe/billing/credit/public-cutover side effects. `verify_live_synthetic_durable_handoff.py` separately records one owner-approved live synthetic durable write/send/status proof: DB write/read/status cleanup passed, one non-customer non-executable SQS handoff passed, rollback controls blocked when enabled, and workers/providers/Stripe/billing/credits/public cutover remained off. `verify_synthetic_durable_worker_lifecycle.py` records local synthetic worker lifecycle proof: queued representation, claim once, duplicate-claim block, processing, retry, terminal failure, terminal completion, terminal readback, DLQ/recovery shape, redacted admin diagnostics, client-safe status, and rollback blocking. `verify_synthetic_failed_job_recovery.py` records local synthetic failed-job recovery proof: safe failure classification, retry exhaustion, redacted DLQ reference shape, represented admin recovery action, client-safe failed status, terminal failed readback, recovered/requeued state, synthetic recovered/completed state, and rollback blocking. `verify_live_no_provider_worker_consumption.py` records one owner-approved compact live no-provider worker proof: synthetic non-customer/non-executable SQS receive passed, durable claim once passed, duplicate claim blocked, processing and terminal status passed, queue delete/ack passed, client/admin diagnostics were redacted, rollback controls blocked when enabled, and providers/media generation/Stripe/billing/credits/customer traffic/public cutover remained off. `verify_aws_backed_synthetic_dlq_recovery.py` records one owner-approved AWS-backed synthetic DLQ recovery proof: synthetic DLQ-shaped message present, non-customer/non-executable markers passed, failure classification passed, retry exhaustion/failed-terminal state represented, admin recovery action represented, recovery/readback passed, diagnostics were redacted, rollback controls blocked when enabled, and customer queue consumption/providers/media generation/Stripe/billing/credits/customer traffic/public cutover remained off. `verify_synthetic_durable_asset_delivery.py` records one owner-approved synthetic durable asset delivery proof: synthetic non-customer/non-executable asset store/read, job link, metadata readback, internal open/download proof, signed URL creation without signed URL exposure, client-safe/admin-actionable redacted views, cleanup, and rollback blocking passed with `synthetic_asset_reference_hash=9fc8e9825ba2`, `synthetic_job_reference_hash=72a3968c271e`, and `object_key_hash_prefix=52e272ee4d0f`; providers/media generation/customer assets/Stripe/billing/credits/customer traffic/public cutover remained off. Future live rehearsal pending owner-approved mode remains the safe operating rule for any future live proof. This still does not prove provider execution, billing/credit execution, real provider-generated final asset delivery, public route cutover, or client delivery.

`verify_production_route_cutover_readiness.py` records the synthetic Production route cutover readiness proof without adding a later migration row. It proves `route_cutover_readiness_passed=true`, `public_cutover_default_disabled=true`, `owner_flags_required=true`, `rollback_kill_switch_blocked_execution=true`, `compatibility_fallback_available=true`, `synthetic_route_reached_aws_durable_path=true`, `synthetic_status_readback_passed=true`, `synthetic_asset_result_metadata_passed=true`, client/admin redaction, and no providers, media generation, Stripe, billing, credits, customer traffic, public cutover, Render removal, or later migration work. This is route-readiness proof only; it does not approve public cutover, paid/customer route traffic, provider execution, billing/credit execution, real provider-generated final asset delivery, or client delivery.
