...
| AWS-15 | Controlled AWS validation and API/status cutover readiness | AWS-15A completed controlled AWS environment validation script/verifier with default no-live/no-network behavior; API route/status cutover remains pending. | Future AWS-15 work can validate AWS credentials/configuration/resources for IAM/STS, RDS, SQS/DLQ, S3, and Secrets Manager before API/status cutover, then wire portal status and backend API reads only after explicit owner approval. | `live_validate_aws_option_a_environment.py`, `verify_live_aws_option_a_environment_validation.py`, future API/status route cutover files | High | `python -X utf8 live_validate_aws_option_a_environment.py`; `python -X utf8 verify_live_aws_option_a_environment_validation.py`; live validation only with `AWS_OPTION_A_LIVE_VALIDATION=true` plus service-specific live validation flags | AWS-15A proves controlled validation defaults to no-live/no-network, requires service-specific live validation flags, prints no secret values, has separately gated SQS test message and S3 test object cleanup paths, avoids migrations/writes/cutover/provider/Stripe/credit mutation, and keeps remaining AWS work compressed into AWS-15 through AWS-20. |
