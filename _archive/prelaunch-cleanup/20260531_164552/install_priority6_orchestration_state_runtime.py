...
STATE_FILE = CORE / "orchestration_state_runtime.py"

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
main_text = MAIN.read_text(encoding="utf-8")

backup = BACKUPS / f"main_before_priority6_orchestration_state_{timestamp}.py"
...
STATE_FILE.write_text(state_code.strip() + "\n", encoding="utf-8")

import_line = "from backend.app.core.orchestration_state_runtime import orchestration_state_readiness, record_orchestration_state, record_orchestration_result, get_orchestration_context, orchestration_recovery_packet\n"
...
if "/admin/orchestration/state-readiness" not in main_text:
    main_text = main_text.rstrip() + "\n\n" + routes + "\n"

MAIN.write_text(main_text, encoding="utf-8")

print("PRIORITY6_ORCHESTRATION_STATE_RUNTIME_INSTALLED")
print(f"Backup: {backup}")
print(f"Created/updated: {STATE_FILE}")
print("Routes:")
print("/admin/orchestration/state-readiness")
print("/admin/orchestration/record-state")
print("/admin/orchestration/record-result")
print("/admin/orchestration/context/{plan_id}")
print("/admin/orchestration/recovery/{plan_id}")