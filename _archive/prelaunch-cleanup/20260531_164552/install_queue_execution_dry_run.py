...
BACKUP = ROOT / "backups" / f"queue_execution_dry_run_before_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

SCRIPT = ROOT / "queue_execution_dry_run.py"
...
def main():
    if SCRIPT.exists():
        ...
        (BACKUP / SCRIPT.name).write_text(SCRIPT.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")

    SCRIPT.write_text(CONTENT, encoding="utf-8")

    print("QUEUE_EXECUTION_DRY_RUN_INSTALLED")
