...
backup = BACKUPS / f"client_page_before_clean_compact_execution_workspace_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tsx"
...
if start == -1:
    raise SystemExit("ERROR: Could not find Activity/Deliverables section marker.")
...
src = src[:start] + new_workspace + src[start:]

PAGE.write_text(src, encoding="utf-8")

print("CLEAN_COMPACT_EXECUTION_WORKSPACE_REBUILT")
...
print("Scrollable installed:", "maxHeight: 250" in src and 'overflowY: "auto"' in src)
print("Old mutation count:", src.count("applyHorizontalExecutionLayout") + src.count("applyPremiumExecutionSectionLayout"))