...
function LargeCard({ f, i }: { f: typeof FEATURES[0]; i: number }) {
  ...
  return (
    <motion.div
      ...
      className={`relative group cursor-default ${i === 0 ? 'md:col-span-7' : 'md:col-span-5'}`}
    >
      <div
        ref={tilt.ref}
        onMouseMove={tilt.onMouseMove}
        onMouseLeave={tilt.onMouseLeave}
        className="glass-ultra rounded-2xl p-8 overflow-hidden h-full"
        style={{
          border: `1px solid ${f.bg}20`,
          minHeight: '240px',
...
function WideCard({ f }: { f: typeof FEATURES[0] }) {
  ...
  return (
    <motion.div
      ...
      className="relative group cursor-default md:col-span-12"
    >
      <div
        ref={tilt.ref}
        onMouseMove={tilt.onMouseMove}
        onMouseLeave={tilt.onMouseLeave}
        className="glass-ultra rounded-2xl p-7 overflow-hidden"
        style={{
          border: `1px solid ${f.bg}18`,
          transition: 'transform 0.15s ease',
          transformStyle: 'preserve-3d',
          willChange: 'transform',
        }}
    ...
    <section id="features" className="section-padding relative overflow-hidden"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
          ...
          {/* Large cards (row 1) — 3D tilt */}
          {largeFt.map((f, i) => <LargeCard key={f.title} f={f} i={i} />)}

          {/* Mid cards (row 2) — 3 equal columns, standard hover */}
          {midFt.map((f, i) => {
            const Icon = f.icon
            return (
              <motion.div
          ...
          {/* Wide card (row 3) — 3D tilt */}
          {wideFt.map((f) => <WideCard key={f.title} f={f} />)}
        </div>
      </div>
    </section>
  )
