...
function OutputCard({ item }: { item: OutputItem }) {
  ...
  function handleEnter() {
    gsap.to(cardRef.current, { y: -4, boxShadow: '0 12px 40px rgba(0,217,255,0.10)', borderColor: 'rgba(0,217,255,0.20)', duration: 0.22, ease: 'power2.out' })
  }

  function handleLeave() {
    gsap.to(cardRef.current, { y: 0, boxShadow: '0 0px 0px rgba(0,0,0,0)', borderColor: 'rgba(255,255,255,0.08)', duration: 0.32, ease: 'power3.out' })
  }
...
export default function AdminLibraryPage() {
  ...
  useGSAP(() => {
    ...
    tl.fromTo(gridRef.current, { y: 24, opacity: 0 }, { y: 0, opacity: 1, duration: 0.45 }, '-=0.25')
  }, { scope: pageRef })
  ...
  useGSAP(() => {
    ...
    if (cards?.length) {
      gsap.fromTo(cards, { scale: 0.96, opacity: 0, y: 14 }, { scale: 1, opacity: 1, y: 0, duration: 0.4, stagger: 0.07, ease: 'back.out(1.4)' })
    }
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontFamily: "'Space Grotesk', sans-serif", color: 'rgba(255,255,255,0.25)', fontSize: '0.82rem', margin: 0 }}>
                Outputs saved by users will appear here
              </p>
            </div>
          </div>
        ) : (
