...
export default function ClientSidebar() {
  ...
  return (
    <aside
      className={`${collapsed ? 'w-14' : 'w-56'} min-h-screen flex flex-col shrink-0 sticky top-0 h-screen transition-all duration-200`}
      style={{
    ...
    >
      ...
      <div className="px-3 py-4 border-b border-white/[0.06] flex items-center justify-between">
        {!collapsed && (
          <Link href="/dashboard" className="flex items-center gap-2.5 group min-w-0">
            <div
              className="w-7 h-7 shrink-0 rounded-lg flex items-center justify-center text-white font-black text-xs transition-all group-hover:scale-105"
        ...
        <button
          ...
          className={`${collapsed ? 'mx-auto mt-2' : ''} shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-white/30 hover:text-white/60 hover:bg-white/[0.06] transition-colors text-xs`}
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              <span className={`text-base leading-none shrink-0 transition-opacity ${active ? 'opacity-100' : 'opacity-50'}`}>{item.icon}</span>
      ...
      {/* Quick actions */}
      <div className={`${collapsed ? 'px-2' : 'px-3'} py-3 border-t border-white/[0.06] space-y-0.5`}>
        {!collapsed ? (
          <>
          ...
          <>
            <Link href="/status" target="_blank" title="System status"
              className="flex justify-center px-2 py-2 rounded-xl text-xs text-gray-500 hover:text-white hover:bg-white/5 transition-all">
              ◉
            </Link>
            <Link href="/pricing" title="Upgrade plan"
              className="flex justify-center px-2 py-2 rounded-xl text-xs text-violet-400 hover:text-violet-300 hover:bg-violet-600/10 transition-all">
              ↑
            </Link>
            <button onClick={signOut} title="Sign out"
              className="w-full flex justify-center px-2 py-2 rounded-xl text-xs text-white/30 hover:text-red-400 hover:bg-red-500/[0.06] transition-all">
              ✕
            </button>
          </>
        )}
